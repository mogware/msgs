...
public class XPull implements SockType {
    @Override
    public int domain() {
    ...
    public SockBase create(Object hint) throws ErrnoException {
        return new XPullImpl(hint);
    }

    @Override
    public boolean isPeer(int socktype) {
        return socktype == Global.PUSH;
    }

    public static class XPullImpl implements SockBase {
        private final Sock sock;
        private final Fq fq;

        public XPullImpl(Object hint) {
            this.sock = (Sock)hint;
            this.fq = new Fq();
        }
